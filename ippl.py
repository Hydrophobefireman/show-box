...
def get_(url, v=True, n=1, season=0):
    ...
    to_screen([
        colors.OKBLUE + "[debug]" + colors.ENDC + "Cookie Jar For %s:" %
        (page.url)
    ], v)
    ...
    div = soup.find(attrs={"id": "list-eps"}) or soup.find(
        attrs={"class": "pas-list"}) or soup.find(attrs={"id": "ip_episode"})
    to_screen(
    ...
    for i in range(n, number_of_eps + 1):
        ...
        toparse = sess.post(
            host + "index.php", data=data_headers, cookies=page.cookies)
        ...
        for t in tgs:
            ...
            to_screen([
                colors.OKBLUE + "[debug]" + colors.ENDC +
                "Working with the configs of", t.string
            ], v)
            to_screen(
...
if __name__ == "__main__":
    number_ = int(
        input(colors.BOLD + "Enter The Number Of URLs:" + colors.ENDC))
