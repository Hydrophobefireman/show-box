...
def check_for_stream_sites(url, ua):
    ...
    if any(a in url for a in s_sites):
        ...
        if re.search(r"https?://.*?putlocker", url) is not None:
            print("try")
            data = list(set([s for s in putlocker(url, ua) if urlcheck(s)]))
        return data
    else:
...
def putlocker(url, ua):
    ...
    def searches(url, source):
        try:
            ...
            if "embed/" in str(g):
                print(g[0])
                d = requests.get(
                    urlparse(url).scheme + "://" + urlparse(url).netloc + g[0]
                ).text
        ...
        try:
            ...
            data_src = json.loads(
                requests.post(
                    ...
                    data={"action": "get_oload_gs", "post_id": p_id},
                ).text
            ).get("src")
            return [data_src]
    ...
    if len(res) == 0 or res is False:
        try:
            ...
            base_url_check = (
                parsed_url.scheme + "://" + parsed_url.netloc + "/tmp_chk.php"
            )
...
def tvzion(url, ua):
    ...
    video = sess.head(
        video[0], headers={"User-Agent": ua, "Referer": url}, allow_redirects=True
    ).url
    if urlcheck(video):
        data.append(video)
        return data
...
def get_final_url(u, ua, url):
    ...
    urls = re.findall(r"(?:(?:https?|ftp)?:\/\/)?[\w/\-?=%.]+\.[\w/\-?=%.]+", source)
    data = []
    for u in urls:
...
def cool_series(url, ua):
    ...
    iframes = [
        s
        for s in re.findall(regex, data, re.IGNORECASE)
        if "http" in s and "vidfast" not in s
    ]
    g_urls = []
    for url in iframes:
        hts = bs(
